...
namespace cnr_control
{
  ...
  bool CartImpedanceController::init(hardware_interface::PosVelEffJointInterface* hw, ros::NodeHandle& root_nh, ros::NodeHandle& controller_nh)
  {
    ...
    try
    {
      ...
      if (!m_controller_nh.getParam("zeroing_sensor_at_startup", m_zeroing_sensor_at_startup))
      {
  ...
  void CartImpedanceController::starting(const ros::Time& time)
  {
    ...
    if(m_zeroing_sensor_at_startup)
      m_init_wrench = true;
    else
      m_init_wrench = false;
...
//                               "\n target = " << m_target.transpose() <<
//                               "\n     Dx = " << m_Dx.transpose() <<
//                               "\n    DDx = " << m_DDx.transpose() <<
//                               "\n wrench = " << m_wrench_of_tool_in_base_with_deadband.transpose());
    }
  }
...
//      ROS_INFO_STREAM_THROTTLE(0.01,"\n          m_wrench_0 = " << m_wrench_0.transpose() <<
//                               "\n          w_s_in_s = " << m_wrench_of_sensor_in_sensor.transpose() <<
//                               "\n          w_t_in_t = " << wrench_of_tool_in_tool.transpose() <<
//                               "\n          w_t_in_b = " << wrench_of_tool_in_base.transpose() <<
//                               "\n w_t_in_b_deadband = " << m_wrench_of_tool_in_base_with_deadband.transpose());
